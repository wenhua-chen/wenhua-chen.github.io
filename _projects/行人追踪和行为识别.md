...
#### 如何做追踪?
  - <span id="基于外观特征相似度">基于<b>外观特征</b>相似度</span>
  ...
  - <span id="基于运动特征相似度">基于<b>运动特征</b>相似度</span>
...
#### 如何提升追踪的准确率和效率?
  - <span id="运动和外观特征一起匹配">二者结合: 基于**运动+外观特征**相似性度的匹配</span>
  ...
  - <span id="先运动后外观特征匹配"><b>更快</b>的匹配方式: 先运动特征匹配, 再外观特征匹配</span>
  ...
  - <span id="逐层匹配机制">提升**稳定性**: 逐层匹配机制</span>
  ...
  - <span id="外观校验">提升<b>准确率</b>: 外观校验</span>
...
#### 如何做行为识别?
  - <span id="人体关键点+规则">人体关键点 + 规则</span>
    - 规则不好确定, 导致结果不准确
    - 算力要求较高
  {% include video id="BV1514y1x7HH" provider="bilibili" %}
  
  - <span id="通过独立帧做分类">通过独立帧做分类</span>
    - 优点
  ...
  - <span id="通过连续帧做分类">通过连续帧做分类</span>
    - 数据预处理
      - 下采样、裁剪、缩放、padding等
    ...
    - 算法
      - [YOWO](https://github.com/wei-tim/YOWO)
        ...
        - 然后空间、时间特征被送入CAFM做融合
      <img src="/assets/images/tracking_recognizing/format,png.png" alt="img" style="zoom:60%;" />{:.align-center}
